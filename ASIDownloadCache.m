...
+ (NSString *)keyForURL:(NSURL *)url
{
    ...
    //リダイレクトで呼ばれる謎プロトコルをここで弾かないと落ちる。法則がないのでベタで書く。
	if (   [urlString length] == 0 
        || [[[urlString  componentsSeparatedByString:@"://"] objectAtIndex:0] isEqualToString:@"itms-appss"] 
        || [[[urlString  componentsSeparatedByString:@"://"] objectAtIndex:0] isEqualToString:@"itmss"] 
    )return nil;
	...
	return [NSString stringWithFormat:@"%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7],result[8], result[9], result[10], result[11],result[12], result[13], result[14], result[15], result[16], result[17], result[18]]; 	
